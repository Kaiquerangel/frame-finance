    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 10, marginBottom: 14 }}>
        {[
          ...
          <Card key={label} style={{ padding: isMobile ? "12px 10px" : 20 }}>
            <div style={{ fontSize: 10, fontWeight: 600, color: "var(--muted)", textTransform: "uppercase", letterSpacing: ".06em", marginBottom: 6 }}>{label}</div>
            <div style={{ fontSize: isMobile ? 14 : 22, fontWeight: 800, color, letterSpacing: "-.02em" }}>{fmt(value)}</div>
          </Card>
        ))}
      </div>
      ...
      <Card style={{ marginBottom: 14 }}>
        <div style={{ fontWeight: 700, fontSize: 14, marginBottom: 12, color: "var(--text)" }}>Novo Lançamento</div>
        {/* Toggle tipo */}
        ...
        {isMobile ? (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            <button onClick={add} disabled={loading} style={{ padding: "12px", borderRadius: 8, border: "none", background: "var(--accent)", color: "#fff", fontWeight: 700, fontSize: 14, cursor: "pointer", opacity: loading ? .7 : 1 }}>
      ...
      <Card>
        <div style={{ display: "flex", gap: 8, marginBottom: 14, flexWrap: "wrap", alignItems: "center" }}>
          ...
          <input value={search} onChange={e => { setSearch(e.target.value); setPage(1); }}
            placeholder="🔍 Buscar..." style={{ ...inp, width: isMobile ? "100%" : 180, fontSize: 12 }} />
