...
export default function Cartoes({ userId }) {
  ...
  const load = async () => {
    const [{ data: c }, { data: i }] = await Promise.all([
      ...
      supabase.from("installments").select("*, purchases(description, category, total_amount, payment_method, has_interest, interest_rate)")
  ...
  // get all months from installments of selected card
  const cardInstallments = useMemo(() =>
    installments.filter(i => i.purchases?.card_id === selected?.id || i.card_id === selected?.id),
  ...
  const usedLimit = selected
    ? installments.filter(i => !i.paid).reduce((a, i) => a + Number(i.amount), 0)
    : 0;
  const limitPct = selected && selected.limit_amount > 0 ? Math.min((usedLimit / selected.limit_amount) * 100, 100) : 0;
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: selected ? "340px 1fr" : "1fr", gap: 16 }}>
        ...
        <div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {cards.map(card => {
              const inst = installments.filter(i => !i.paid);
              const used = inst.reduce((a, i) => a + Number(i.amount), 0);
        ...
        {selected && (
          <div>
            <Card style={{ marginBottom: 16 }}>
              <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 16, color: "var(--text)" }}>Fatura — {selected.name}</div>
