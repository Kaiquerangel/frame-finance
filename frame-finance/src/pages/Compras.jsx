...
function calcInstallments(total, numInst, hasInterest, monthlyRate, purchaseDate, card) {
  ...
  // Protege contra data inválida
  const parsedDate = new Date(purchaseDate + "T12:00:00");
  if (isNaN(parsedDate.getTime())) {
    for (let i = 0; i < numInst; i++) {
      installments.push({ installment_number: i + 1, amount: parseFloat(pmt.toFixed(2)), due_date: today() });
    }
  ...
  for (let i = 0; i < numInst; i++) {
    ...
    if (card && card.due_day && card.closing_day) {
      const closingThisMonth = new Date(parsedDate.getFullYear(), parsedDate.getMonth(), card.closing_day);
      const monthsAhead = parsedDate > closingThisMonth ? i + 2 : i + 1;
      dueDate = new Date(parsedDate.getFullYear(), parsedDate.getMonth() + monthsAhead - 1, card.due_day);
    } else {
...
function bestDayToBuy(card) {
  ...
  const bestDay = card.closing_day + 1 > 28 ? 1 : card.closing_day + 1;
  return { bestDay, closingDay: card.closing_day, dueDay: card.due_day };
}
...
export default function Compras({ userId, onNavigate }) {
  ...
  // Carrega dados — useCallback para evitar recriações desnecessárias
  const load = useCallback(async () => {
    const [{ data: p }, { data: i }, { data: c }, cats] = await Promise.all([
      supabase.from("purchases")
        .select("*, cards(name,color,due_day,closing_day)")
        .eq("user_id", userId)
        .order("purchase_date", { ascending: false }),
      supabase.from("installments")
        .select("*, purchases(description, card_id, cards(name))")
        .eq("user_id", userId)
        .order("due_date"),
  ...
  // Preview — só calcula se a data for válida
  const preview = useMemo(() => {
    if (!form.total_amount || !form.purchase_date) return null;
    const parsed = new Date(form.purchase_date + "T12:00:00");
  ...
  // Handler da data — valida antes de atualizar o estado
  const handleDateChange = (e) => {
    const val = e.target.value;
    // Aceita vazio (limpando o campo) ou data válida no formato YYYY-MM-DD
    if (!val || /^\d{4}-\d{2}-\d{2}$/.test(val)) {
      setForm(f => ({ ...f, purchase_date: val || today() }));
    }
  };
  ...
  const add = async () => {
    ...
    if (!form.purchase_date || !/^\d{4}-\d{2}-\d{2}$/.test(form.purchase_date)) return;
    ...
    if (!error && data) {
      ...
      // Atualiza estado local imediatamente para evitar inconsistência
      setPurchases(prev => [{ ...data, cards: card }, ...prev]);
    }

    setForm(EMPTY_FORM);
    setPage(1);
    await load(); // re-fetch completo para garantir consistência
    setLoading(false);
  };
  ...
  const saveEdit = async () => {
    const updatedFields = {
      ...
      purchase_date: editForm.purchase_date || editForm.purchase_date,
    };
    await supabase.from("purchases").update(updatedFields).eq("id", editId);
    // Atualiza local imediatamente
    setPurchases(prev => prev.map(p => p.id === editId ? { ...p, ...updatedFields } : p));
    setEditId(null);
    await load();
  };

  const del = async (id) => {
    // Remove localmente antes do fetch para UX instantânea
  ...
  return (
    <div>
      <EmptyBanner pageId="compras" onNavigate={onNavigate}
        message="Registre aqui qualquer compra que você fez parcelada ou não. Veja como funciona." />

      {!isMobile && (
        <div style={{ marginBottom: 24 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <h1 style={{ fontWeight: 800, fontSize: 22, color: "var(--text)", letterSpacing: "-.02em" }}>Compras</h1>
            <button onClick={() => { sessionStorage.setItem("ff_help_section", "compras"); onNavigate("aprendendo"); }} title="Como usar" style={{ background: "none", border: "none", cursor: "pointer", color: "var(--accent)", fontSize: 16, padding: "2px 4px", fontWeight: 700 }}>?</button>
          </div>
          <p style={{ color: "var(--muted)", fontSize: 13, marginTop: 2 }}>Registre compras no crédito, débito, pix ou dinheiro</p>
        </div>
      )}
      ...
      <Card style={{ marginBottom: 16 }}>
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
            {PAYMENT_METHODS.map(pm => (
              <button key={pm.id} onClick={() => setForm(f => ({ ...f, payment_method: pm.id, installments: "1", has_interest: false, interest_rate: "" }))} style={{
                padding: isMobile ? "9px 12px" : "7px 14px", borderRadius: 8, border: "1.5px solid",
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 10 }}>
          <div>
            ...
            <input value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))}
              placeholder="Ex: iPhone 15, Geladeira..." maxLength={150} style={inp} />
          </div>
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr 1fr" : "1fr 1fr 1fr", gap: 10 }}>
            ...
            <div style={ isMobile ? { gridColumn: "1 / -1" } : {} }>
              ...
              {/* Usa type="date" com validação segura */}
              <input
                type="date"
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8, paddingTop: 2 }}>
                  ...
                  {form.has_interest
                    ? <input type="number" value={form.interest_rate}
                        ...
                        placeholder="Ex: 2.5" style={{ ...inp, flex: 1 }} />
        ...
        {bestDay && (
          <div style={{ background: "var(--greenbg)", borderRadius: 8, padding: "9px 14px", marginBottom: 10, fontSize: 12, color: "var(--green)" }}>
            Melhor dia para comprar: dia {bestDay.bestDay}. Você terá mais tempo antes do fechamento (dia {bestDay.closingDay}) e do vencimento (dia {bestDay.dueDay}).
          </div>
        )}
          ...
          <div style={{ background: "var(--accentbg)", borderRadius: 8, padding: "10px 14px", marginBottom: 12, fontSize: 13 }}>
            <div style={{ display: "flex", gap: isMobile ? 12 : 24, flexWrap: "wrap" }}>
              ...
              {preview.totalInterest > 0 && (
                <span>Juros: <strong style={{ color: "var(--red)" }}>{fmt(preview.totalInterest)}</strong></span>
              )}
            </div>
          </div>
        )}

        <button onClick={add} disabled={loading || !form.description || !form.total_amount || !form.category} style={{
          padding: isMobile ? "13px 0" : "9px 22px",
          width: isMobile ? "100%" : "auto",
          borderRadius: 8, border: "none", background: "var(--accent)",
          color: "#fff", fontWeight: 700, fontSize: 13, cursor: "pointer",
          opacity: (loading || !form.description || !form.total_amount || !form.category) ? .5 : 1,
        }}>
      ...
      {/* KPIs */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: isMobile ? 10 : 12, marginBottom: 16 }}>
        <Card style={{ padding: isMobile ? "14px" : 20 }}>
          <div style={{ fontSize: 10, fontWeight: 600, color: "var(--muted)", textTransform: "uppercase", letterSpacing: ".06em", marginBottom: 6 }}>Total em aberto</div>
          <div style={{ fontSize: isMobile ? 16 : 22, fontWeight: 800, color: "var(--red)" }}>{fmt(totalDebt)}</div>
          <div style={{ fontSize: 12, color: "var(--muted)", marginTop: 4 }}>{installments.filter(i => !i.paid).length} parcelas</div>
        </Card>
        <Card style={{ padding: isMobile ? "14px" : 20 }}>
          <div style={{ fontSize: 10, fontWeight: 600, color: "var(--muted)", textTransform: "uppercase", letterSpacing: ".06em", marginBottom: 6 }}>Vence este mês</div>
          <div style={{ fontSize: isMobile ? 16 : 22, fontWeight: 800, color: "var(--accent)" }}>{fmt(monthDebt)}</div>
          <div style={{ fontSize: 12, color: "var(--muted)", marginTop: 4 }}>{monthInstallments.filter(i => !i.paid).length} no mês</div>
        </Card>
      </div>

      {/* Abas */}
      <div style={{ display: "flex", gap: 2, marginBottom: 14, background: "var(--surface)", borderRadius: 10, padding: 4, border: "1px solid var(--border)", width: "fit-content" }}>
        ...
        <Card>
          <div style={{ display: "flex", gap: 10, marginBottom: 14, alignItems: "center", flexWrap: "wrap" }}>
            <input value={search} onChange={e => { setSearch(e.target.value); setPage(1); }}
              placeholder="Buscar compra..." style={{ ...inp, flex: 1, minWidth: 140, fontSize: 12 }} />
            <span style={{ fontSize: 12, color: "var(--muted)", flexShrink: 0 }}>{filteredPurchases.length} compra(s)</span>
          </div>
          ...
          {filteredPurchases.length === 0
            ...
            : paginated.map((p, i) => {
              ...
              const pInsts = installments.filter(inst => inst.purchases?.card_id === p.card_id && inst.purchase_id === p.id);
              const paid = pInsts.filter(i => i.paid).length;
              return (
                <div key={p.id} style={{ borderBottom: i < paginated.length - 1 ? "1px solid var(--border)" : "none" }}>
                  {editId === p.id ? (
                    <div style={{ padding: "10px 0", display: "flex", flexDirection: isMobile ? "column" : "row", gap: 8 }}>
                      ...
                      <div>
                        <div style={{ fontSize: 11, color: "var(--muted)", marginBottom: 4 }}>Data da compra</div>
                          ...
                          <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 2 }}>
                            ...
                            {p.installments > 1 && (
                              <span style={{ marginLeft: 5 }}>
                                · {paid}/{p.installments} parcelas pagas
                                {p.has_interest ? ` · c/ juros` : ""}
                              </span>
                            )}
                          </div>
                        </div>
          ...
          {totalPages > 1 && (
            <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: 8, marginTop: 16, paddingTop: 16, borderTop: "1px solid var(--border)" }}>
              <button onClick={() => setPage(p => Math.max(1, p-1))} disabled={page===1} style={{ padding: "6px 14px", borderRadius: 7, border: "1px solid var(--border)", background: "var(--bg)", color: page===1?"var(--muted)":"var(--text)", cursor: page===1?"not-allowed":"pointer", fontSize: 13 }}>←</button>
              <span style={{ fontSize: 13, color: "var(--muted)" }}>{page} / {totalPages}</span>
              <button onClick={() => setPage(p => Math.min(totalPages, p+1))} disabled={page===totalPages} style={{ padding: "6px 14px", borderRadius: 7, border: "1px solid var(--border)", background: "var(--bg)", color: page===totalPages?"var(--muted)":"var(--text)", cursor: page===totalPages?"not-allowed":"pointer", fontSize: 13 }}>→</button>
            </div>
          )}
        </Card>
      )}
            ...
            {monthInstallments.length === 0
              ...
              : monthInstallments.map((inst, i) => (
                <div key={inst.id} style={{
                  ...
                  padding: "10px 0", borderBottom: i < monthInstallments.length - 1 ? "1px solid var(--border)" : "none",
