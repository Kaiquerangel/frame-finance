...
export default function Perfil({ userId, profile, onClose, onUpdate }) {
  ...
  const saveProfile = async () => {
    ...
    if (firstName.trim().length < 2) { setError("Nome muito curto."); setLoading(false); return; }
    if (lastName.trim().length < 2)  { setError("Sobrenome muito curto."); setLoading(false); return; }
  ...
  const savePassword = async () => {
    ...
    if (newPass.length < 8) { setError("A senha precisa ter pelo menos 8 caracteres."); return; }
    if (!/[A-Z]/.test(newPass) || !/[0-9]/.test(newPass)) {
      setError("Use pelo menos uma letra maiúscula e um número."); return;
    }
    setLoading(true);
    ...
    <>
      <div onClick={e => e.target === e.currentTarget && onClose()} style={{
        ...
        <div style={{
          ...
          {/* Header */}
          <div style={{ padding: isMobile ? "14px 20px 0" : "24px 24px 0", display: "flex", justifyContent: "space-between", alignItems: "center", flexShrink: 0 }}>
          ...
          {/* Avatar + info */}
          <div style={{ padding: isMobile ? "14px 20px 0" : "20px 24px 0", display: "flex", alignItems: "center", gap: 14, flexShrink: 0 }}>
          ...
          {/* Tabs */}
          <div style={{ display: "flex", padding: isMobile ? "12px 20px 0" : "16px 24px 0", gap: 4, borderBottom: "1px solid var(--border)", marginTop: 4, flexShrink: 0, overflowX: "auto" }}>
            {TABS.map(t => (
              <button key={t.id} onClick={() => { setActiveTab(t.id); setError(""); setSuccess(""); }} style={{
                ...
                marginBottom: -1, transition: "all .12s", whiteSpace: "nowrap", flexShrink: 0,
              }}>{t.label}</button>
            ))}
          </div>

          {/* Tab content — scrollável */}
          <div style={{ padding: isMobile ? "16px 20px 24px" : "20px 24px 24px", overflowY: "auto", flex: 1 }}>
            ...
            {activeTab === "info" && (
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                  <div>
                    <Label>Nome</Label>
                    <input value={firstName} onChange={e => setFirstName(e.target.value)} maxLength={50} style={inp} placeholder="Nome" />
                  </div>
                  <div>
                    <Label>Sobrenome</Label>
                    <input value={lastName} onChange={e => setLastName(e.target.value)} maxLength={50} style={inp} placeholder="Sobrenome" />
                  </div>
                </div>
                <div>
                  <Label>Nick</Label>
                ...
                <div>
                  ...
                  <textarea value={bio} onChange={e => setBio(e.target.value)}
                    placeholder="Uma frase sobre você..." rows={2} maxLength={200}
            ...
            {activeTab === "senha" && (
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                <div>
                  ...
                  <input type="password" value={newPass} onChange={e => setNewPass(e.target.value)} style={inp} placeholder="Mínimo 8 caracteres" />
                </div>
                {newPass.length > 0 && (
                  <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                    {[
                      { label: "8+ chars",  ok: newPass.length >= 8 },
                      { label: "Maiúscula", ok: /[A-Z]/.test(newPass) },
            ...
            {activeTab === "prefs" && (
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                ...
                <div style={{ borderRadius: 10, padding: 14, border: "1px solid var(--red)", background: "var(--redbg)", marginTop: 4 }}>
                  ...
                  }} onClick={() => setError("Para excluir sua conta, entre em contato pelo e-mail: suporte@kradev.com.br")}>
                    Excluir minha conta
                  </button>
                </div>
              </div>
